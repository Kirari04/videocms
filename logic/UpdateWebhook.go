...
func (s *Service) UpdateWebhook(validated *models.WebhookUpdateValidation, userID uint) (status int, response string, err error) {
	...
	if res := s.Deps.DB.Save(&webhook); res.Error != nil {
