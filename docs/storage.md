...
To reconnect the same bucket, select **Mount**. To migrate the bucket, detach the mount, edit its endpoint, bucket, prefix, or credentials, and mount it again. VideoCMS scans canonical per-file object prefixes and automatically relinks matching unavailable file IDs. The **Scan and reconnect files** action can also preview the number of matches before applying the relink.

Connecting a brand-new mount also performs this scan, so a replacement mount can recover records whose previous mount no longer exists. Relinking updates database records only; it never copies, modifies, or deletes objects.
